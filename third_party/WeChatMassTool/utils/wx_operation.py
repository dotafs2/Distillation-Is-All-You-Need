...
class WxOperation:
    ...
    def __goto_chat_box(self, name: str) -> bool:
        ...
        time.sleep(0.5)  # wait for search results to load

        # 微信 4.x: ListControl 控件树变了，直接尝试用 ListControl 匹配
        try:
            ...
            # 微信 4.x fallback: 搜索结果加载后直接按回车选第一个
            self.wx_window.SendKey(key=auto.SpecialKeyNames['ENTER'], waitTime=0.3)
            time.sleep(0.3)
            return True
    ...
    def send_msg(self, name, msgs=None, file_paths=None, add_remark_name=False, at_everyone=False,
        ...
        # 设置输入框为当前焦点
        # 微信 4.x: EditControl 可能没有 Name 属性匹配, fallback 到无名 EditControl
        self.input_edit = self.wx_window.EditControl(Name=name)
        if not self.input_edit.Exists(1, 0.2):
            self.input_edit = self.wx_window.EditControl()
